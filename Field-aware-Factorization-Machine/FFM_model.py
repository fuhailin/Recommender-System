...
def inference(input_x, input_x_field, zeroWeights, oneDimWeights, thirdWeight):
    ...
    secondValue = tf.reduce_sum(tf.multiply(
        oneDimWeights, input_x, name='secondValue'))
...
if __name__ == '__main__':
    ...
    y_ = inference(input_x, trainx_field, zeroWeights,
                   oneDimWeights, thirdWeight)

    l2_norm = tf.reduce_sum(
        tf.add(
            tf.multiply(lambda_w, tf.pow(oneDimWeights, 2)),
            tf.reduce_sum(tf.multiply(
                lambda_v, tf.pow(thirdWeight, 2)), axis=[1, 2])
        )
    )
        ...
        for i in range(total_plan_train_steps):
            for t in range(all_data_size):
                ...
                saver.save(sess, os.path.join(
                    MODEL_SAVE_PATH, MODEL_NAME), global_step=steps)
                writer = tf.summary.FileWriter(os.path.join(
                    MODEL_SAVE_PATH, MODEL_NAME), tf.get_default_graph())
                writer.close()
        #
